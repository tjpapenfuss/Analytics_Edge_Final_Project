...
#cv.tree = train(review_overall ~ +brewery_name+beer_style+beer_abv, #not running to avoid computational time (Edgar)
                 data = df,
                 method="rpart",
                 tuneGrid=data.frame(cp=seq(0, 0.0005, 0.0001)),
                 trControl=trainControl(method="cv", number=10))
#cv.tree #The results recommend cp = 0? Is this right?  #not running to avoid computational time (Edgar)
...
#rf.cv = train(review_overall ~ +brewery_name+beer_style+beer_abv, #not running to avoid computational time (Edgar)
              #data = df,
...
#rf.cv  #not running to avoid computational time (Edgar)
...
pr <- prediction(p, test$reviewoverall)
